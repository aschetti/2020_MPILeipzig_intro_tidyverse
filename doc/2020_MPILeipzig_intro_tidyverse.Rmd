---
title: '<center> <h1> Reproducible data analysis using R and the ```tidyverse``` </h1> </center>'
...
* click “New Directory”
* click “New Project”
* type in the name of the directory to store your project, e.g., “YOURNAME_2020_MPILeipzig_intro_tidyverse”
...
In other words, we are still inside our *R Project* directory (*`r here()`*) but we can load and save files inside the respective subfolders.
...
A string of *1*s and *2*s. It's easy to get a glimpse of the unique values of this variable because there are not many observations. When you have more observations, better use the function ```unique```:
...
* filter out outlier participants as well as all participants who rated *water* after *neutral* imagery as _**lower than -10**_
...
Something is missing... ah, error bars! Display the standard deviation with `geom_errorbar`.   
...
barplot_4 <-
  barplot_3 +
  geom_errorbar(aes(
    # half the standard deviation below and and above the mean
    ymin = mean - sd / 2,
    ymax = mean + sd / 2
  ),
...
These colors are terrible... let's use a better-looking color palette. The package ```viridis``` uses colors that are easier to distinguish for people with color blindness.
...
barplot_6 <-
  ...
  scale_y_continuous("", # y-axis: no title
    limits = c(-16, 16), # y-axis: min/max values
    breaks = seq(-16, 16, 4) # y-axis: tick marks
  ) +
...
RDI plots display **R**aw data, **D**escriptives, and **I**nferential statistics altogether. Nothing is hidden by useless bars, everything is shown so that readers can make up their minds. For example, the plot[^6] below shows:
...
As you can see, we installed ```ggpirate``` in a different way: we used the function ```install_github``` from the package ```devtools```. That's because ```ggpirate``` is on [**GitHub**](https://github.com/mikabr/ggpirate) but not on [**CRAN**](https://cran.r-project.org/), the official repository for R packages. Also, note that we did not explicitly load the whole ```devtools``` library, but we simply picked out the function we needed.

Don't forget to load this newly installed package `r emo::ji("wink")`
...
```{r RDIplot, warning = FALSE, message = FALSE}
...
```{r owl, echo = FALSE, out.width = "80%", fig.cap = "*How to draw an owl. Source: [Know Your Meme](https://knowyourmeme.com/memes/how-to-draw-an-owl)*"}

include_graphics("images/owl.png")
...
```{r RDIplot_save, message = FALSE}

ggsave(
  filename = "RDIplot.png",
  ...
  dpi = 600 # 600 dpi is often requested for publications
)
...
* a statistically significant difference (*p* = `r format(posthoc_att$test$pvalues[2], digits = 2)`) between ratings of **beer** and **water** after **negative** imagery --> after negative advertisement, people (still) rate beer as more pleasant than water
* a statistically significant difference (*p* = `r format(posthoc_att$test$pvalues[6], digits = 1, scientific = FALSE)`) between ratings of **water** after **neutral** compared to **negative** imagery --> water was rated as less pleasant after negative compared to neutral advertisement

Interestingly, **beer** ratings between **negative** and **neutral** imagery are *not* statistically significant (*p* = `r  format(posthoc_att$test$pvalues[1], digits = 3)`). This **might** indicate that the experimental manipulation did not have the intended effect, although such a conclusion cannot be inferred from the non-significant *p*-value alone.
...
Run bootstrapped effect sizes (calculate **Pearson's _r_** instead of Hegdes's *g*) and display the results in console.
...
There are ways to (almost) ensure computational reproducibility. The best option I have found so far is to create an R package with data, preprocessing and analysis scripts, and files that document the specific version of each package used for the computations. This package can be embedded into a **container**, a sort of virtual machine that contains an operating system with its own software, libraries, and configuration files. Learning these skills require some time but, in my opinion, it's worth the time and effort. If you are interested, check out [**```rrtools```**](https://github.com/benmarwick/rrtools).

Another good (and low-effort `r emo::ji("smile")`) option is to use the function ```sessionInfo```, which lists important information related to your operating system and libraries as well as R and package versions.
...
[^1]: Hopefully you're not familiar with _**pretending**_ to collect data... if so, please tell your story by writing a [book](https://nick.brown.free.fr/stapel/FakingScience-20161115.pdf).

[^2]: As you may imagine, today we won't have time to cover all the amazing things you can do with these packages. Also, they are a gift that keeps on giving: I have been using them for a while and I keep discovering useful functions. If you want to learn more, read [R for Data Science](https://r4ds.had.co.nz/) by [Garrett Grolemund](https://www.oreilly.com/pub/au/5570) and [Hadley Wickham](http://hadley.nz/).
...
[^8]: If you prefer **not** to bootstrap your effect sizes, you can use the ```MBESS``` package (a useful tutorial can be found [here](https://osc.centerforopenscience.org/2014/03/06/confidence%20intervals/)).

[^9]: A clear explanation of how to use ```bootES``` is provided by the authors of the package in their [paper](https://doi.org/10.3758/s13428-013-0330-5).
