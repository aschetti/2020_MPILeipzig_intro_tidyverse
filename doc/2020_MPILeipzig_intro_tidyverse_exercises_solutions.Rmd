...
subtitle: '<center> <h1> Solutions </h1> </center>'
...
```{r plot, echo = FALSE, out.width = "80%", fig.cap = "*RDI plot generated during the workshop.*"}

include_graphics("images/RDIplot.png")

```

Create a plot similar to what is shown above (with ```wine``` instead of ```beer```) and save it in the subfolder ```doc/images```.

```{r exercise_3_1, warning = FALSE, message = FALSE}
...
RDIplot_ex_3_1 <-
  ...
  geom_pirate(
    bars = FALSE,
    cis = TRUE,
    lines = TRUE, lines_params = list(color = "black", alpha = .4),
...
Run bootstrapped effect sizes (calculate **Pearson's _r_** instead of Hegdes' *g*) and display the results in console.
